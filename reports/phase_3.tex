...
prediction at zero almost exactly ($60.00\%$ empirical vs.\ $59.95\%$
...
a conservative result attributable to the shrinkage estimator itself.
\end{abstract}
...
$\gamma = 2{,}000/801 \approx 2.497$) is a $2{,}000 \times 2{,}000$
...
likely to produce interpretable spectral outliers.
...
support $[0.34, 6.66]$) vs.\ the Marchenko-Pastur density
(Eq.~\ref{eq:mp-density}, $\gamma=2.497$), rescaled to the conditional
...
correlation matrix, $\gamma=2.497$, theoretical support $[0.337, 6.657]$).}
...
The point-mass prediction is confirmed almost exactly: $60.00\%$ of the
$2{,}000$ eigenvalues are numerically zero, against a theoretical
prediction of $1 - 1/\gamma = 59.95\%$ (the small excess reflects the
centered data matrix's rank being $n-1=800$ rather than $n=801$). Of the
remaining nonzero eigenvalues, $27$ ($1.35\%$ of all $2{,}000$) exceed the
theoretical bulk edge $b=6.657$ (Table~\ref{tab:spectral-outliers}), the
...
$\chi^2_{2000}$ $97.5\%$-quantile outlier threshold marked.}
...
exceed the formal outlier threshold of 2125.84).}
\label{tab:mahalanobis-top10}
\begin{tabular}{lrr}
\toprule
Sample & Cancer type & Squared Mahalanobis distance \\
\midrule
sample\_560 & BRCA & 803.64 \\
sample\_250 & BRCA & 801.97 \\
sample\_599 & LUAD & 801.87 \\
sample\_322 & KIRC & 801.78 \\
sample\_789 & KIRC & 801.69 \\
sample\_497 & BRCA & 801.21 \\
sample\_577 & LUAD & 800.21 \\
sample\_389 & BRCA & 799.99 \\
sample\_406 & BRCA & 799.91 \\
sample\_343 & LUAD & 799.90 \\
\bottomrule
\end{tabular}
\end{table}
...
distances range from $723.13$ to $803.64$ (mean $778.10$, std $11.57$),
\emph{all} far below the $\chi^2_{2000}$ $97.5\%$ threshold of $2125.84$:
...
raw distance (Table~\ref{tab:mahalanobis-top10}) skew toward BRCA (5 of 10,
vs.\ its $37.45\%$ class share) and LUAD (3 of 10, vs.\ $17.6\%$), with no
PRAD or COAD samples appearing -- with only 10 samples this is not a
statistically robust pattern, but is noted descriptively.
...
rather than being read as ``the data has no outliers.'' The Ledoit-Wolf
target shrinks the covariance toward $\mu I$ with $\mu = \mathrm{tr}(S)/p$
-- effectively an isotropic estimate with a single, averaged
variance per gene. Because the true correlation-matrix spectrum is
extremely heavy-tailed (Table~\ref{tab:spectral-outliers}: eigenvalues from
near-zero to $318.3$), this isotropic blending inflates the estimated
variance along the many near-zero directions relative to their true (near-zero)
variance far more than it deflates the variance along the few
large-eigenvalue directions, so $\hat\Sigma_{\text{LW}}^{-1}$
systematically under-weights genuinely informative deviations. This is
visible directly in the data: the mean observed $D^2$ ($778.1$) is only
$38.9\%$ of its nominal expectation under the $\chi^2_{2000}$ null ($2{,}000$),
showing the shrinkage-regularized distances are compressed well below what
the classical chi-squared calibration assumes, not that the underlying
data lacks structure. Mahalanobis distance under heavy shrinkage is
therefore better used here as a \emph{relative ranking} tool
(Table~\ref{tab:mahalanobis-top10}) than as a formally calibrated
significance test -- a limitation of applying an asymptotic Gaussian
threshold to a deliberately regularized, non-asymptotic $d \gg n$ estimator.
