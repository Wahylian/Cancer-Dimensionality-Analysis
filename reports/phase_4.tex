...
$97.5\%$ $\chi^2_{2000}$ threshold). We close with an explicit discussion of
...
exceed the Marchenko--Pastur theoretical bulk edge $b = 6.657$ at aspect
ratio $\gamma = 2.497$ -- eigenvalues that cannot be explained by an
...
Correlation dimension $\hat D_2$ ($R^2$) & 7.73 (1.000) & 49.02 (0.998) & 4.63 (1.000) \\
$k$-NN MLE mean, $k \in \{5,10,20\}$ & 29.36 & 619.35 & 5.27 \\
\quad$k$-NN MLE ($k=5$) & 33.16 & 656.95 & 5.44 \\
\quad$k$-NN MLE ($k=10$) & 29.55 & 618.75 & 5.28 \\
\quad$k$-NN MLE ($k=20$) & 25.37 & 582.36 & 5.09 \\
Kaiser-criterion PCA elbow (components) & 100 & 386 & 5 \\
Curvature-criterion PCA elbow (components) & 2 & 799 & 4 \\
PCA components for 90\% variance & 392 & 688 & 5 \\
PCA components for 95\% variance & 548 & 742 & 5 \\
PCA components for 99\% variance & 732 & 788 & 5 \\
MP spectral outlier count (top-2,000 genes, $\gamma=2.497$) & 27 & -- & -- \\
...
Mahalanobis-flagged outliers ($\alpha=0.025$, $n=801$) & 0 & -- & -- \\
\bottomrule
\end{tabular}
\end{table}
...
samples (5 BRCA, 3 LUAD, 2 KIRC, $0$ PRAD/COAD, Phase 3, Table~7) -- carries
...
\begin{itemize}
  ...
  \item \textbf{Permutation-calibrated Mahalanobis threshold.} As noted in
  Phase 3's Limitations, the $\chi^2_{2000}$ threshold assumes asymptotic
  multivariate normality of $D^2$ under the fitted covariance, an
  approximation Phase 3 shows is violated under heavy shrinkage (observed
  mean $D^2 = 778.1$ is only $38.9\%$ of the nominal $\chi^2_{2000}$
  expectation). A permutation-based null (recomputing $D^2$ under
